...
# ── Guidance mode ────────────────────────────────────────────────────────────
# 'MAVLINK'   — follow pre-planned MAVLink waypoints (gate map required)
...
class Controller:
    ...
    def _handle_init(self):
        if 'pos' not in self.data:
            return
        now = time.time()
        needs_gate_map = GUIDANCE == 'MAVLINK'
    ...
    def _handle_fly(self):
        ...
        cv_fresh = GUIDANCE in ('CV_LIVE', 'CV_PLAN') and cv_pos is not None and cv_age < CV_STALE_TIMEOUT
        if cv_fresh:
